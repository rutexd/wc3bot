...
const EN: T = T {
    ...
    quiet_on: "{start}–{end}",
    quiet_off: "All time",
    qh_now_in: "🔕 Now silent — notifications paused",
    qh_now_out: "🔔 Now active — notifications allowed",
    quiet_disabled: "⏰ Notification window is disabled.\n\nNotifications are sent at any time.",
...
const RU: T = T {
    ...
    quiet_on: "{start}–{end}",
    quiet_off: "Всё время",
    qh_now_in: "🔕 Сейчас тихо — уведомления не приходят",
    qh_now_out: "🔔 Сейчас активное окно — уведомления приходят",
    quiet_disabled: "⏰ Время уведомлений не настроено.\n\nУведомления приходят в любое время.",
